...
* **quad_gazebo.launch:** Launches the simulation engine and initializes the stack. Can support multiple robots.
...
* **visualization.launch:** Launches the visualization drivers to map data topics into RViz-readable topics. Can also be configured to launch RViz, PlotJuggler, and/or an rqt dashboard.

     - **`namespace`** The namespace of the robot for proper remapping. Default: `robot_1`.
     - **`visualization`** Specifies whether or not to launch RViz and optional visualization options. Default: `true`.
     - **`live_plot`** Specifies whether to launch PlotJuggler for live plotting of data. Default: `false`.
     - **`dash`** Specifies whether or not to launch the rqt dashboard. Default: `false`.
