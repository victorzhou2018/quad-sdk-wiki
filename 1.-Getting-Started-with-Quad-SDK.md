...
catkin build
```
6. (**Only for users using free license of HSL solver**): Go to ```nmpc_controller/src/nmpc_controller.cpp``` and change line 208 the linear solver setting from ```ma57``` to ```ma27```.
